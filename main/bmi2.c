...
#define BMI2_CHIP_ID        	0x24u /*!< Bmi270 sensor value*/
#define BMI2_REG_ID            	0x00u
...
static esp_err_t bmi2_write(bmi2_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *const data_buf, const uint8_t data_len)
{
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Start i2c failed at write func!!");
...
static esp_err_t bmi2_write_config(bmi2_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *const data_buf_1, const uint8_t *const data_buf_2, const uint8_t data_len)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ret = i2c_master_start(cmd);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Start i2c failed at write config func!!");
    ret = i2c_master_write_byte(cmd, sens->dev_addr | I2C_MASTER_WRITE, true);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write byte i2c failed at write config func!!");
    ret = i2c_master_write_byte(cmd, reg_start_addr, true);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write byte i2c failed at write config func!!");
    ret = i2c_master_write(cmd, data_buf_1, data_len, true);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write i2c failed at write config func!!");
    ret = i2c_master_write(cmd, data_buf_2, data_len, true);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write i2c failed at write config func!!");
    ret = i2c_master_stop(cmd);
    if(ret != ESP_OK) ESP_LOGE(TAG, "Stop i2c failed at write config func!!");
    ret = i2c_master_cmd_begin(sens->bus, cmd, 1000 / portTICK_PERIOD_MS);
    i2c_cmd_link_delete(cmd);
...
static esp_err_t load_config(bmi2_handle_t sensor, const uint8_t* input)
{
	...
	for (uint16_t i = 0; i < 4096; i++) {
...

		{
		    ...
		    if(ret != ESP_OK) ESP_LOGE(TAG, "Write byte i2c failed at load config func!!");
		}
...
static esp_err_t bmi2_get_deviceid(bmi2_handle_t sensor)
{
	esp_err_t ret;
	uint8_t id;
	ret = bmi2_read(sensor, BMI2_REG_ID, &id, 1);
	if(ret != ESP_OK) ESP_LOGE(TAG, "Read failed at get device id func");
	if(id != BMI2_CHIP_ID) return ESP_ERR_INVALID_RESPONSE;
...
esp_err_t bmi2_begin(bmi2_handle_t sensor)
{
	...
	ret = bmi2_get_deviceid(sensor);
	if(ret != ESP_OK) {
		ESP_LOGE(TAG, "Invalid sensor id, bmi270 = 0x24");
	...
	ret = bmi2_write(sensor, soft_reset_addr, &soft_reset_val, 1);
	vTaskDelay(pdMS_TO_TICKS(500));
    if(ret != ESP_OK) ESP_LOGE(TAG, "Reset failed at begin func!!");
//    assert(ESP_OK == ret);
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write byte i2c failed at begin func!!");
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Write byte i2c failed at begin func!!");
    if(0x01 != internal_status) ESP_LOGE(TAG, "Status failed at begin func!!");


    //set working mode
	uint8_t pwr_ctrl_val = 0x04;	//only enable accelerometer
...
//    assert(ESP_OK == ret);
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Read byte i2c failed at begin func!!");
    switch(acce_range){
    case 0:
...
esp_err_t bmi2_get_raw_acce(bmi2_handle_t sensor, bmi2_raw_acce_value_t *const raw_acce_value)
{
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Read byte i2c failed at get raw acce func!!");
...
esp_err_t bmi2_get_acce(bmi2_handle_t sensor, bmi2_acce_value_t *const acce_value)
{
    ...
    if(ret != ESP_OK) ESP_LOGE(TAG, "Get raw acce func failed at get acce func!!");
